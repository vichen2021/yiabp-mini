...
- **用户不信任手写的基础 CRUD 代码**：任何未通过 `module-generator`、`crud-generator-plus`、`field-sync` 生成的基础脚手架代码（包括后端 5 层模块、DTO、Service，以及前端 `api/`、`views/` 列表/表单/弹窗），用户都视为不可信。
- **一旦发现绕过 skill 手写**：用户会**整体回退所有相关代码**，无论这些代码看起来多"能跑"、多"干净"。
- **后果非常严重**：开发进度直接拖慢、已经消耗的大量 token 全部作废、上下游依赖与前后端契约被迫重做、信任成本大幅上升。
