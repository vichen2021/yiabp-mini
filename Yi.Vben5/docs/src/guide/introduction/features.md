...
```
module/{模块名}/
├── Yi.Module.{Module}.Domain.Shared/          # 共享层
├── Yi.Module.{Module}.Domain/                 # 领域层
├── Yi.Module.{Module}.Application.Contracts/  # 应用契约层
├── Yi.Module.{Module}.Application/            # 应用层
└── Yi.Module.{Module}.SqlSugarCore/           # 基础设施层
```

`Yi.Framework.*` 只用于框架基础设施，业务模块统一使用 `Yi.Module.*`。
...
~~旧版 `code-gen` 模块和 `Yi.Abp.Tool` 模块生成器~~ 已在 2.0 移除，相关能力改为通过 Skills 实现。
