...
```
<worktree>\.claude\skills\code-standard-review\scripts\code-standard-review.ps1
```

例如 master-branch 下：
```
E:\WCG\Private\WJ-CMS-Agent-Work\WorkTrees\master-branch\.claude\skills\code-standard-review\scripts\code-standard-review.ps1
```

脚本支持任意 `-Root` 工作副本路径，不要求在脚本所在 worktree 内执行。

## 工作副本路径

```
master-branch:  E:\WCG\Private\WJ-CMS-Agent-Work\WorkTrees\master-branch
main-branch:    E:\WCG\Private\WJ-CMS-Agent-Work\WorkTrees\wj-cms-branches\main-branch
roco-branch:    E:\WCG\Private\WJ-CMS-Agent-Work\WorkTrees\wj-cms-branches\roco-branch
```
...
向用户确认要检查哪个工作副本，或基于上下文判断：
- 用户提及 master / 基础底座 → `master-branch`
- 用户提及 main / 产品基础 → `main-branch`
- 用户提及 roco → `roco-branch`
- 不明确时直接询问
...
使用 PowerShell 7 (`pwsh`) 调用：

```powershell
pwsh -File "E:\WCG\Private\WJ-CMS-Agent-Work\WorkTrees\master-branch\.claude\skills\code-standard-review\scripts\code-standard-review.ps1" -Root "<工作副本绝对路径>" [其他开关]
```

不要使用 `cd` 进入脚本目录，参数中直接传入绝对路径。
...
2. FAIL 项的修复方向（不要直接修改业务代码，除非用户授权）
...
- 修复 FAIL 时严格遵守分支边界：基础底座问题在 `master-branch` 修，产品通用问题在 `main-branch` 修，产品差异问题在 `roco-branch` 修。
